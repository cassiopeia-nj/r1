...
L_total = 400;	 % infomation bits plus tail bits(Frame size)
...
% Total number of bit errors for all iterations
   ...
   if rem(nframe,10)==0 | nframe==framelim
% Bit error rate
...
% Display intermediate results in process  
         fprintf('************** Eb/N0 = %5.2f db***********%d**************\n', EbN0db(nEN),nEN/length(EbN0db));
...
% Save intermediate results 
   ...
   filename=strcat('changeRate',num2str(QQ),num2str(TT));
  save (filename,'data');
